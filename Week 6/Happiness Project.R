...
for (file in 2015:2020) {
  ...
  # graph
  ggplot(file, aes(Rank, Score)) + geom_point()
}
...
#Y2020 = Y2020 %>% select(c( "Country", "Region", "Score", "Logged GDP", "Social Support", "Health (Life Expectancy)", "Freedom", "Generosity", "Corruption"))


# Y2019$`Happy_Level` = cut(Y2019$`Overall rank`,
#                                breaks=c(0,25,102,140,Inf),
#                                include.lowest=TRUE,
#                                labels=c("Ecstatic","Happy","Neutral","Sad"))

#dat = select(Y2019, -c("Overall rank", "Country or region"))


## EXPLORATORY ANALYSIS
# Get a feel for the data.
hist(Y2020$Score, breaks = seq(2,8, 0.5), xlab = "Score", main = "Happiness Score Distribution")
#ggplot(Y2020, aes(x=Score)) + geom_histogram(color = "darkblue", fill = "lightblue", bins = 30)
boxplot(Y2020$Score)
...
dat = select(Y2020, -c("Country", "Rank", "Dystopia + residual"))
...
dat = Y2020 %>% rename("Health" = "Health (Life Expectancy)") %>% remove_col_spaces()


rf = randomForest(Score ~ ., data = dat, proximity = TRUE)
visualize_model(rf, dat$Score, "Random Forest")

dat = select(Y2020, -Country) # We don't want the models acciententally try
...
# Little new section
dat = select(Y2019, -c("Overall rank", "Country or region", "Happy_Level"))
